...
from ideas.ui_uploadLyrsTrees import Ui_Dialog
from ideas.NwkToJSON import Parser
...
# .............................................................................
   ...
   def showTreeFileDialog(self):
      ...
      """
      settings = QSettings()
      dirName = settings.value( "/UI/lastShapefileDir" ).toString()  ### have to take this out when in QGIS
      ...
      if treePath == "":
         ...
         if os.path.exists(treePath):
            try:
               ...
               #self.treeLayout.addWidget(self.progressbar)
               ...
               #print self.workspace.getWSforUser(self.client._cl.userId)
   ...
   def processFolder(self,dirname):
      ...
      full = [os.path.join(dirname,f) for f in os.listdir(dirname)
               ...
               (os.path.splitext(f)[1] == RASTER_EXTENSION or
                os.path.splitext(f)[1] == SHAPEFILE_EXTENSION) and
      ...
      shpList = [os.path.splitext(os.path.basename(f))[0] 
                 for f in full if os.path.splitext(f)[1] == SHAPEFILE_EXTENSION]
      ...
      for x, rec in enumerate(self.table.tableView.model().data):
         ...
         if namePath.has_key(rec[0]): # this check between tree and folder
            ...
            if os.path.splitext(os.path.basename(path))[1] == SHAPEFILE_EXTENSION:
      ...
      for crs, record in zip(crsMatch,enumerate(self.table.tableView.model().data)):
         if record[1][0] not in names  and crs:
            if self.addUpload(names, ids, record, skipUpload=False):
               ...
               self.addMtrxIdxToTree(record[1][0])
               successcount += 1
         if not crs:
...
#  
   ...
   d = UploadDialog(None,client=client,epsg='4326')
   #d = AdvancedAlgo()
   d.show()
   sys.exit(qApp.exec_())               
