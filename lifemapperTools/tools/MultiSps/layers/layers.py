...
from lifemapperTools.tools.MultiSps.layers.ui_Layers import Ui_Dialog
...
class OTTSearchResult(object):
   ...
   def __init__(self,hintDict):
      try:
         self.ottId = hintDict["ott_id"]
         self.name = hintDict["unique_name"]
      except Exception, e:
         print "hint result ",str(e)
         self.ottId = '-999'
         self.name = 'test'
...
class LayerController(Ui_Dialog):
...
# .............................................................................
   ...
   def showTreeFileDialog(self):
      ...
      extensions = ['*.%s' % (x) for x in ['nhx','tre']]
      filetypestr = 'newick files (%s)' % (' '.join(extensions))
      print filetypestr
   ...
   def convertNewick(self):
      ...
      if treePath == "":
         ...
         if os.path.exists(treePath):
            ...
            #treeJSON = treePath
            #jsonStr = open(treeJSON).read()
            #phyloDict = json.loads(str(jsonStr))
            #   
            #self.phylo = phyloDict
            #self.lengthOfExistingData =  len(self.table.tableView.model().data)
            #print "starting len ",self.lengthOfExistingData
